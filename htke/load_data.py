...
class Data:
    ...
    def read_icir7():
        ...
        for txt in globbed_files:
            ...
            # Analysis set up to use the column heading 'Relative Time'
            tmp.rename(columns={'DateTime':'Relative Time'}, inplace = True)

            # Remove the decimal points for seconds
            tmp['Relative Time'] = pd.to_datetime(tmp['Relative Time']).apply(lambda x: x.replace(microsecond=0))

            # dt.time removes the date
            tmp['Relative Time'] = pd.to_datetime(tmp['Relative Time'], format = '%H:%M:%S').dt.time.astype('string')
